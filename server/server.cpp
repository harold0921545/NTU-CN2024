            ...
            if (login){
                ...
                for (int i = 0; i < user_count; ++i){
                    if (user[i].username == username && user[i].password == password){
                        ...
                        while (1){
                            user[i].chat_sockfd = accept(server_sockfd, (sockaddr *) &chat_client_addr, &chat_client_addr_len); // no race condition, mutex
                            if (user[i].chat_sockfd > 0) { // retry every 2 seconds
                                break;
                            }
                            sleep(2);
                        }
